...
class QuestionsManager
  ...
  def createNewQuestion(question_type: String, options: Array, translation_type: String, key_word: String,
                        key_word_morse: String, correct_answer: String, question_description: String, level: Level)
    if validateParams(question_type: question_type, options: options, translation_type: translation_type,
                      key_word: key_word, key_word_morse: key_word_morse, correct_answer: correct_answer, question_description: question_description, level: level)
      case question_type
      when 'Multiple_choice'
        @question = Question.create!(description: question_description, level: level,
      ...
      when 'To_complete'
        @question = Question.create!(description: question_description, level: level,
                                     questionable: To_complete.create!(keyword: key_word, toCompleteMorse: key_word_morse))
        Answer.create!(answer: correct_answer, correct: true, question: @question)
      when 'MouseTranslation'
        @question = Question.create!(description: question_description, level: level,
  ...
  def validateParams(question_type: String, options: Array, translation_type: String, key_word: String,
                     key_word_morse: String, correct_answer: String, question_description: String, level: Level)
    if !level.nil? && Level.exists?(level.id)
      ...
      when 'Translation'
        ...
        when 'morse_translation'
          question_description.match?(/.*[.\-]+.*$/) && correct_answer.match?(/\A[a-zA-ZñÑáéíóúÁÉÍÓÚ0-9 ]+\z/)
        when 'spanish_translation'
          question_description.match?(/\A[a-zA-ZñÑáéíóúÁÉÍÓÚ0-9 ]+\z/) && correct_answer.match?(/\A[.\- ]+\z/)
        else
          false
        end

      when 'To_complete'
        (key_word.match?(/[a-zA-ZñÑáéíóúÁÉÍÓÚ0-9]+/) and key_word_morse.match?(/^[.-]+$/))
